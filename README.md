# Projeto-Catálogo
Catálogo com produtos feito tudo do zero, com base no projeto do canal Sujeito Programador
...
## O projeto tem horário de funcionamento no javascript, recomendo alterar ou remover para testes. Quando o site está fora do horário do script, ele desabilita a opção de finalizar o carrinho.
 ...
 # ATENÇÃO:
#### Se for rodar o projeto na sua máquina, baixar as bibliotecas necessárias (dá pra publicar direto na netlify ou vercel e editar dando publish mesmo pra não precisar ir atrás disso)

 
 ### O código foi desenvolvido com as tecnologias: Html, Css, Node, Tailwind e JavaScript.
 Para mudar qualquer coisa na aparência do código, é só alterar pelo próprio HTML (não mude os ids declarados no html pra não dar conflito no backend)
 Caso queira, você pode mudar todas as fotos e adicionar quantos produtos quiser.
...
# Para adicionar algo na sessão 2, copie o código e adapte a seguir, colando dentro da sessão <!--GRID BEBIDAS--> :
```
<!--BEBIDA ITEM-->
